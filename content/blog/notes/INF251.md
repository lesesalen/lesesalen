...
- 2D curve example:
  - y = k \* x + d -> linear slope with k steepness and d startheight
    - Line through (0, d)^T
- Sphere example:
...
- Parametric: x(a, b) = (c_x + r _ cos(a) _ sin(b), c_y + r _ sin(a) _ sin(b), ...)
- Implicit: (x - c)^2 = r^2
...
```
1: x(u) = a_x * u^3 + b_x * u^2 + c_x * u + d_x, 0 <= u <= 1
```
...
```
2: x(u) = [u^3, u^2, u, 1] * [a_x, b_x, c_x, d_x] = U * C
```
...
```
x(u) = sum(k, 0 to 3) of g_k * BF_k(x)
```

where g_k is the geometric contraint parameters and BF_k is the _blending functions_ or _basis functons_. This is the normal representation of the generalized spline **B-Spline**.
...
- Incoming light:
  ...
  - `B_i = E_i + P_i * sum(j=1,...,n){B_j * F_ij}`
  - `F_ij` = Form factor
...
- Sum of form factors should sum to 1 (part of whole hemisphere).
  - `sum(j=1,...,n){F_ij} = 1`
- `A_i * F_ij = A_i * F_ji` <- Uniform light direction
- `F_ii = 0` <- No self radiance
...
F_ij = cos(phi_i) * cos(phi_j) * A_j / pi * r^2
```
...
- `x(f) = integral(inf, -inf){x(f) * e^(-i * 2 * Pi * ft)} dt`, i = imaginary part
...
- Dirac impulse = `f(x) = inf if x = 0, 0 otherwise`
  - Comb function

## Nyquist-Shannon Theorem

- A sampled function can be reconstructed if:
  - It was band-limited before sampling: `x(f) = 0 for all f = B`
    - "perfect looping cutoff"
  - And it was sampled above the Nyquist rate: `f_s > 2*B`
...
- **Polar coordinates**: `x = r * cos(alpha), y = r * sin(alpha)`
  - Inverse transformation from Cartesian to polar coordinates: `r = sqrt(x^2 + y^2), alpha = tan^-1 (y / x)`
...
- Scalar product
  - Angle between vectors
    - `a * b / |a| * |b| = cos(ab)`
  - Projection:
    - `a * b = |a| * cos(ab)` = a projected on b (distance)
- Vector product
  - Normal
    - `a x b = |a| * |b| * sin(ab) * n_w`
...
x' = r * cos(angle + angle'), y' = r * sin(angle + angle')
...
- Matrices are associative
  - Can be grouped: `(M_1 * M_2) * M_3 = M_1 * (M_2 * M_3)`
